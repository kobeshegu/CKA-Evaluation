    ...
    def iterate_images():
        for idx, fname in enumerate(input_images):
            ...
            try:
                ...
                print("failed to load image: {}".format(fname))
            if image.mode == 'RGBA':
...
def make_transform(
    ...
    def center_crop_wide(width, height, img):
        ...
        if int(np.round(in_h * out_w / out_h)) < in_w:
            ch = np.int(np.round(in_h * out_w / out_h))
            img = img[:, (img.shape[1] - ch) // 2 : (img.shape[1] + ch) // 2]
        else:
            ch = np.int(np.round(in_w * out_h / out_w))
            img = img[(img.shape[0] - ch) // 2 : (img.shape[0] + ch) // 2]
        # if img.shape[1] < width or ch < height:
        #     print(img.shape[0], img.shape[1],img.shape[2], width, ch, height)
        #     return None
        # img = img[(img.shape[0] - ch) // 2 : (img.shape[0] + ch) // 2]
        img = PIL.Image.fromarray(img, 'RGB')
        img = img.resize((width, height), PIL.Image.LANCZOS)
