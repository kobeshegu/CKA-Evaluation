    ...
    def forward(self, inputs, **kwargs):
        if not isinstance(inputs, list):
            ...
            if not hasattr(self, 'mean'):
                self.mean = torch.as_tensor([0.485, 0.456, 0.406], dtype=inputs.dtype, device=inputs.device).view(-1, 1, 1)
            if not hasattr(self, 'std'):
                self.std = torch.as_tensor([0.229, 0.224, 0.225], dtype=inputs.dtype, device=inputs.device).view(-1, 1, 1)
        ...
        for end_idx in idx_crops:
            _out = self.forward_backbone(torch.cat(inputs[start_idx: end_idx]).to(inputs[0].device))
            if start_idx == 0:
                output = _out
            else:
                output = torch.cat((output, _out))
            start_idx = end_idx
        out = self.forward_head(output)
        return out
